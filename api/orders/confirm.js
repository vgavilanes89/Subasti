...
export default async function handler(req, res) {
  ...
  if (order.status !== ORDER_STATUS.SHIPPED) {
    return res.status(409).json({ error: 'NOT_SHIPPED' });
  }
  ...
  const rows = await sql`
    UPDATE orders SET
      ...
      received_at = to_timestamp(${receivedAt} / 1000.0),
      confirmation_due_at = to_timestamp(${receivedAt + ESCROW_WINDOW_MS} / 1000.0)
    WHERE id = ${order.id}
    RETURNING *
  `;
