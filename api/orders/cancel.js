...
export default async function handler(req, res) {
  ...
  if (order.payment_due_at && Date.now() <= new Date(order.payment_due_at).getTime()) {
    return res.status(409).json({ error: 'CANNOT_CANCEL' });
  }
  ...
  const rows = await sql`
    UPDATE orders SET
      ...
      cancelled_at = to_timestamp(${Date.now()} / 1000.0)
    WHERE id = ${order.id}
    RETURNING *
  `;
  ...
  if (buyer) {
    await createNotification({
      ...
      type: 'order_status',
      title: `Tu pedido "${order.item_title}" fue cancelado`,
      body: 'El vendedor canceló el pedido porque no se completó el pago a tiempo.',
