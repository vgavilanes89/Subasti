    ...
    if (!seller) {
        return (
            <div>
                ...
                <div className="bg-white p-8 rounded-lg shadow-md text-center">Seller not found.</div>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border mb-6">
                <h1 className="text-3xl font-bold">{seller.profileName}{L.title}</h1>
                <div className="flex items-center gap-2 mt-2">
                    <StarRating rating={averageRating} />
                    <span className="text-gray-600">({averageRating.toFixed(1)} from {seller.reviews.length} reviews)</span>
                </div>
                        ...
                        return (
                            <div key={index} className="border-b pb-4">
                                <div className="flex items-center mb-1">
                                    ...
                                    <p className="ml-2 font-bold">{reviewer?.profileName || 'Anonymous'}</p>
                                </div>
                                <p className="text-gray-600">{review.comment}</p>
                            </div>
                        );
                    })}
                    {seller.reviews.length === 0 && <p className="text-gray-500">No reviews yet.</p>}
                </div>
            </div>

            <h2 className="text-2xl font-bold mb-4">{seller.profileName}{L.listings} ({sellerItems.length})</h2>
